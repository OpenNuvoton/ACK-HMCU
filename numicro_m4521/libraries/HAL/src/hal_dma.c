...
static int HAL_DMA_FillTimeout(PDMA_T *pdma, int channelid, int timeout_us)
{
    ...
    if (timeout_us && channelid < NU_PDMA_CH_MAX)   // limit
    {
        uint32_t u32TOutCnt =  timeout_us / 100;

        PDMA_DisableTimeout(1 << channelid);
        PDMA_EnableInt(channelid, PDMA_INT_TIMEOUT);    // Interrupt type
        PDMA_SetTimeOut(channelid, 1, u32TOutCnt);

        return 0;
    }
    else
    {
        PDMA_DisableInt(channelid, PDMA_INT_TIMEOUT);    // Interrupt type
        PDMA_DisableTimeout(1 << channelid);
    }
...
int HAL_DMA_FillDescription(int channelid, uint32_t u32Peripheral, uint32_t data_width, uint32_t addr_src, uint32_t addr_dst, int32_t length, uint32_t timeout)
{
    ...
    PDMA_SetTransferMode(channelid,
                         u32Peripheral,
                         0,
                         0);
    PDMA_SetTransferCnt(channelid,
                        (data_width == 8) ? PDMA_WIDTH_8 : (data_width == 16) ? PDMA_WIDTH_16 : PDMA_WIDTH_32,
                        length);

    PDMA_SetTransferAddr(channelid,
                         addr_src,
                         u32SrcCtrl,
                         addr_dst,
                         u32DstCtrl);

    PDMA_SetBurstType(channelid,
                      PDMA_REQ_SINGLE,
                      0);

    PDMA_EnableInt(channelid, PDMA_INT_TRANS_DONE);
